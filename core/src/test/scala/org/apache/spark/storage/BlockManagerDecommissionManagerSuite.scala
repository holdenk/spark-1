...
class BlockManagerDecommissionManagerSuite extends SparkFunSuite with Matchers {
  ...
  test("test shuffle and cached rdd migration without any error") {
    ...
    val migratableRDDResolver = mock(classOf[MigratableRDDResolver])

    val storedBlock1 =
      new ReplicateBlock(RDDBlockId(0, 0), Seq(BlockManagerId("replicaHolder", "host1", bmPort)), 1)

    when(migratableRDDResolver.getMigratableRDDBlocks())
      .thenReturn(Seq(storedBlock1))

    val migratableShuffleBlockResolver = mock(classOf[MigratableResolver])
    registerShuffleBlocks(migratableShuffleBlockResolver, Set((1, 1L, 1)))
    val peerProvider = mock(classOf[BlockManagerPeerProvider])

    when(peerProvider.getPeers(mc.any()))
      .thenReturn(Seq(BlockManagerId("exec2", "host2", 12345)))

    val bmDecomManager = new BlockManagerDecommissionManager(
      sparkConf,
      blockTransferService,
      migratableRDDResolver,
    ...
    verify(migratableRDDResolver, times(1)).migrateBlock(storedBlock1)
