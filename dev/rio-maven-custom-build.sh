...
function exit_with_usage {
  echo "builder.sh <scala-version 2.10|2.11>"
  echo ""
  echo ""
  exit 1
}
...
POM_SPARK_HADOOP_VERSION=$("$MVN" help:evaluate -Dexpression=hadoop.version $@ 2>/dev/null\
    | grep -v "INFO"\
    | tail -n 1)
POM_SPARK_HIVE=$("$MVN" help:evaluate -Dexpression=project.activeProfiles -pl sql/hive $@ 2>/dev/null\
    | grep -v "INFO"\
